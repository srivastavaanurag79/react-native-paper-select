...
and run `npm i` in the root directory to install the required dependencies:

```
npm i
```

> While it's possible to use [`yarn`](https://classic.yarnpkg.com/lang/en/), the tooling is built around [`npm`](https://github.com/npm/cli), so you'll have an easier time if you use `npm` for development.
...
* `test`: adding or updating tests, e.g. adding integration tests using detox.
...
Our pre-commit hooks verify that the linter when committing.
